...
Parallel::~Parallel() {
	...
	for (vector<Command*>::iterator it = commands->begin(); it!= commands->end(); ++it) {
		ofRemoveListener((*it)->onComplete, this, &Parallel::_complete);
	}
	//destroy();
...
//--------------------------------------------------------------
void Parallel::_complete(Command& command) {
	ofRemoveListener(command.onComplete, this, &Parallel::_complete);
...
void Parallel::executeFunction(Command* command) {
	...
	if (getNumCommands() > 0) {
		vector<Command*>* commands = &getCommands();
		for (vector<Command*>::iterator it = commands->begin(); it!= commands->end(); ++it) {
			ofAddListener((*it)->onComplete, this, &Parallel::_complete);
			(*it)->execute();
		}
	} else {
		notifyComplete();
